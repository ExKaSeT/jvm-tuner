...
import last.project.jvmtuner.props.ZGCProps;
...
public class ZGCService implements TuningModeService {
    ...
    public TuningTask start(TuningTestProps testProps) {
        var task = taskService.createTask(testProps, TuningMode.Z_GC);
    ...
    public void process(long taskId, UUID testUuid) {
        ...
        data = sequentialModeService.process(data, zgcProps.getRetryOnFailCount(), testProps, task, test, taskTest);
